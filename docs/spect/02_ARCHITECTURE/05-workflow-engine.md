...
> **وضعیت پیاده‌سازی (2026-07-13):** یک DAG engine v1 در
> `apps/openon4net-runtime` پیاده‌سازی شد (RT-048، roadmap فاز ۳ هفته
> ۳۱-۳۲)، طبق `09_TASKS/08-scope-guardrails-mvp.md`'s «Workflow Engine با
> YAML ساده (بدون Visual Builder)». محدودسازی‌های عمدی v1 نسبت به این سند:
>
> - فقط ۵ step type ساخته شد: `agent`/`tool`/`human`/`parallel`/`condition`
>   (`packages/shared/src/schemas/workflow.ts`) — `loop` و `notification`
>   هنوز نیستند.
> - **Visual Builder، template library، و parser YAML عمداً نیستند** —
>   definition به‌صورت JSON مستقیم ذخیره می‌شود (§۲'s «Visual Builder» و
>   «Workflow Parser: YAML → DAG» بلوک‌ها هنوز پیاده‌سازی نشدند). UI
>   (`web/app/workflows/page.tsx`) یک textarea خام JSON است.
> - فقط اجرای دستی (`POST /v1/workflows/:id/run`) — `scheduled`/`event`
>   trigger هنوز نیستند.
> - `condition` step فقط مقایسه‌ی ساختاریافته (`eq`/`neq`/`gt`/`lt`/`gte`/`lte`
>   روی یک dot-path در context) دارد — **بدون زبان expression رشته‌ای**
>   (سند مثال `"$result.amount > 10000"` را می‌زند؛ عمداً پیاده‌سازی نشد،
>   ریسک امنیتی eval رشته‌ای).
> - `parallel` فقط یک سطح است — sub-stepها فقط `agent`/`tool` می‌توانند
>   باشند، نه `parallel`/`condition` تودرتو.
> - اجرا سه زیرسیستم از قبل ساخته‌شده را مستقیم استفاده می‌کند، نه
>   زیرساخت موازی: `tool-dispatcher.ts`'s `executeTool()` برای step نوع
>   `tool`، `ChatService.chat()` برای `agent`، `ApprovalService.create()`
>   برای `human` (تعمیم‌یافته در RT-042).
> - جزئیات کامل + تست‌ها: `docs/spect/DONE.md`'s بخش «Phase 3 — هفته
>   ۲۹-۳۲ — RT-044..048».
