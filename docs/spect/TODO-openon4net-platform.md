...
| CP-037 | کاتالوگ Provider (~۱۰-۱۵ تای اول، داده نه کد) + دسته‌بندی — روی همون آداپتورهای OpenAI-compatible/Anthropic-native موجود در `@o2n/llm-providers`             | لیست دقیق و محورهای دسته‌بندی موکول به پیاده‌سازی (پیشنهاد اولیه Claude در جلسه ۸)                                                                                                |
...
> **وابستگی به CP-013:** سقف بودجه‌ی هر توکن (CP-036) باید از همون معماری
> wallet/pricing موجود (`pricing.ts`, `wallet-service.ts`) استفاده کنه، نه یک
> مسیر پرداخت موازی جدید — هماهنگ با CP-013 (Billing واقعی) در فاز پیاده‌سازی.
