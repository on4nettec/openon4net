...
- **CP-SP-01: کامل.** T-CP-001 (اسکلت gateway/migrations/docker-compose) ✅، T-CP-002 (ثبت در `pnpm-workspace.yaml` ریشه) ✅، T-CP-003 (migrations) ✅، T-CP-004 (app skeleton) ✅، T-CP-005 (issue activation key) ✅، T-CP-006 (check-in) ✅، T-CP-008 (CI) ✅. **فقط T-CP-007 (کلاینت سمت Runtime) عمداً معلق مونده** — چون به `openon4net-runtime` دست می‌زنه، نیاز به اجازه صریح جدا داره.
...
- **2026-07-10 — CP-001 (اولین دور end-to-end واقعی، از `docs/spect/TODO-openon4net-control-plane.md`):** دیتابیس واقعی `o2n_control_plane` روی همون Postgres بیرونی ساخته و migrate شد، gateway واقعاً بالا اومد، و کل مسیر issue-key → check-in → credit (با idempotency واقعاً تست‌شده) → wallet/transactions → admin/organizations با curl واقعی تأیید شد، به‌علاوه مسیرهای خطا (401 برای کلید غلط ادمین/activation). یک مشکل زیرساختی preexisting هم کشف شد (collation-version mismatch روی `template1` همون Postgres) — دورش زده شد با `TEMPLATE template0`، ربطی به کد CP نداره. جزئیات کامل در `docs/spect/DONE.md`.
...
- **باقی‌مونده برای اطمینان کامل:** تعامل واقعی مرورگر با فرم‌های `web/` (CP-002 در TODO)، و rate limiter به سقف نرسیده تا 429 واقعی دیده بشه (CP-007 در TODO). جزئیات در `docs/spect/TODO-openon4net-control-plane.md` و `docs/spect/DONE.md`.
...
| T-CP-007 | Runtime-side: activation client + periodic check-in                                                  | E-061 | M   | T-CP-006     | تغییر متقابل لازم در `apps/openon4net-runtime/gateway`  |
...
**Risks:** T-CP-007 نیازمند تغییر در ریپوی دیگر (`openon4net-runtime`) است؛ باید هماهنگ با وضعیت فعلی SP-03 آن ریپو انجام شود تا رگرسیون ایجاد نکند.
