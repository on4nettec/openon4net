...
## ۱.۲) Feature Gating (Plan-based / Licensing)

برخی قابلیت‌ها باید **بر اساس پلن/لایسنس** در Runtime enforce شوند (هم در API و هم در UI):

- **Programmer Agent + مهارت‌های توسعه نرم‌افزار** فقط برای سازمان‌هایی فعال باشد که «AI Gateway» را خریده‌اند (یا پلن/افزونه مربوط را دارند).
- اگر سازمان لایسنس را ندارد:
  - Agent role = `programmer` در ایجاد/ویرایش قابل انتخاب نباشد.
  - Skill/Pluginهای دسته Development که نیازمند AI Gateway هستند، در Marketplace/Install UI نمایش داده نشوند یا به‌صورت read-only با پیام ارتقا نمایش داده شوند.
  - هرگونه تلاش API برای فعال‌سازی/اجرا با `permission_denied` یا `feature_not_available` رد شود و audit شود.

> هدف: جلوگیری از دسترسی ناخواسته به Agent/Skillهای هزینه‌زا و تضمین مدل درآمدی/کنترل هزینه.
