...
| CP-009 | commit کردن کد داخل ریپوی خودِ `openon4net-Platform` (الان همه‌چیز فقط working-tree diff هست، آخرین commit اونجا هنوز `"add readme"`)                                                                                                                                                                       | طبق قانون: فقط با درخواست صریح commit می‌کنم                                                                                                                                                     |
| CP-010 | تصمیم `.gitignore`/ثبت `apps/openon4net-Platform` به‌عنوان git submodule واقعی در ریشه (مثل `openon4net-runtime`)                                                                                                                                                                                           | الان عمداً هنوز توی `.gitignore` ریشه‌ست؛ برداشتنش یعنی git این پوشه رو embedded repo می‌بینه                                                                                                    |
...
| CP-051 | بسته‌بندی «AI Gateway on4net» به‌عنوان یک پلاگین نصب‌شدنی رسمی (روی قرارداد RT-112 در `TODO-openon4net-runtime.md`) — نه یک قابلیت built-in جدا                                 | وابسته به RT-112. **تأیید شد (جلسه ۱۲): این همون توکن AI Gateway (RT-101، جلسه ۸) است، نه یک مفهوم جدا** — «نصب پلاگین» = «تنظیم توکن». **یادداشت جلسه ۱۵:** این اولین نمونه‌ی الگوی «جفت پلاگین Platform+Runtime» بود، فقط اسمش گذاشته نشده بود — نیمه‌ی Platform همینه، نیمه‌ی Runtime همون RT-112..131 |
