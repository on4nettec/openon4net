# Control Plane Backlog — `openon4net-Platform` (Plane 2)

> این سند مکمل `02_backlog.md` / `03_sprint-plan.md` است، نه جایگزین آن‌ها. آن دو سند فقط `apps/openon4net-runtime` (Plane 1) را پوشش می‌دهند؛ این‌جا Epic/Story/Task/Sprint مخصوص `apps/openon4net-Platform` تعریف می‌شود تا namespace اسپرینت‌های Runtime (`E-0##`/`T-0##`/`SP-##`) قاطی نشود.
>
> Spec مرجع:
>
...
> «routing/failover بین چند مدل مسئولیت انحصاری `openon4net-Platform` است... باید به بک‌لاگ Control Plane منتقل شود» (T-011 منسوخ در Runtime)
...
پیاده‌سازی به‌صورت کاملاً مستقل داخل `apps/openon4net-Platform/` جلو رفته — بدون هیچ تغییری در `apps/openon4net-runtime`.
...
- US-600: به عنوان Developer، `apps/openon4net-Platform/{gateway,web,migrations,docker-compose.yml}` را طبق layout استاندارد داشته باشم.
...
2. ~~`tools/migrations/0003_billing.sql` منسوخ است.~~ **انجام شد (2026-07-09):** `apps/openon4net-Platform/README.md` اصلاح شد تا به `apps/openon4net-Platform/migrations/` (هنوز ساخته نشده، T-CP-003) اشاره کند و صراحتاً بگوید DB مستقل از Runtime است.
