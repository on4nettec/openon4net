...
| CP-009 | ✅ بسته — stale، از قبل ارضا شده                                                                                            | ریپو از خیلی وقت پیش کامیت‌های واقعی زیادی داره (بیش از CP-032..CP-066)، همیشه فقط با درخواست صریح. ایشو #13 با توضیح بسته شد (۲۰۲۶-۰۷-۲۹)                                                       |
| CP-010 | ⏳ تصمیم `.gitignore`/ثبت `apps/openon4net-Platform` به‌عنوان git submodule واقعی در ریشه (مثل `openon4net-runtime`)        | یک تصمیم معماری با تبعات hard-to-reverse روی ساختار git ریشه — عمداً بدون تصمیم کاربر پیش نرفتم                                                                                                  |
| CP-011 | ✅ Admin auth واقعی‌تر (چند ادمین جدا + audit)                                                                              | ساخته و تست شد (۲۰۲۶-۰۷-۲۹). جزئیات: docs/spect/DONE.md                                                                                                                                          |
| CP-014 | ✅ فیکس race در `turbo.json` ریشه                                                                                           | ساخته و تست شد (۲۰۲۶-۰۷-۲۹) — `typecheck.dependsOn` حالا `["^build", "build"]`. جزئیات: docs/spect/DONE.md                                                                                       |
...
| CP-038 | ✅ موتور انتخاب هوشمند provider: اگه ترتیب دستی کاربر روی توکن ست شده همون استفاده بشه، وگرنه امتیازدهی ترکیبی هزینه/در‌دسترس‌بودن                                                                                                                                                                                                                                                                                                                         | فرمول: شمارش ساده‌ی خطای اخیر (نه circuit-breaker کامل) + قیمت. فقط منطق انتخاب/امتیازدهی ساخته شد — اجرای واقعی completion هنوز وصل نشده (نیاز به credential اصلی provider سمت Platform که هیچ تسکی نساخته). جزئیات: docs/spect/DONE.md                                   |
| CP-039 | ✅ داشبورد مصرف در Platform: مصرف کل + به‌تفکیک هر Workspace + هزینه‌ی هر provider                                                                                                                                                                                                                                                                                                                                                                         | گسترش listUsage() به سطح workspace شد. به‌تفکیک «هر کاربر» ساخته نشد — درخواست AI Gateway به هویت یک کاربر انسانی وصل نیست (فقط org/token)، بدون تغییر پروتکل Runtime قابل‌ساخت نیست. جزئیات: docs/spect/DONE.md                                                           |
| CP-040 | ✅ نمای عمومی محبوبیت provider در landing page (پرمصرف‌ترین provider، آمار تجمیعی/بی‌نام)                                                                                                                                                                                                                                                                                                                                                                  | `GET /v1/landing/provider-popularity`، بدون auth، فقط تجمیعی (بدون org_id/workspace_id). جزئیات: docs/spect/DONE.md                                                                                                                                                        |
| CP-041 | ❌ **منسوخ شد (جلسه ۱۵) — جایگزین: CP-054.** ~~اتصال Workspace به Google Drive، به‌شکل انتزاع «storage connector»~~                                                                                                                                                                                                                                                                                                                                        | دامنه‌ی CP-054 بزرگ‌تره: یک پلاگین Google با چند سرویس (نه فقط Drive) + consent انتخابی + چند حساب                                                                                                                                                                         |
| CP-042 | ✅ کمیسیون Platform روی مصرف — درصد قابل‌تنظیم توسط ادمین Platform، روی قیمت خام هر provider (به‌ازای هر یک میلیون توکن)                                                                                                                                                                                                                                                                                                                                   | تنظیم + محاسبه ساخته شد (جدول singleton، applyCommission، نمایش در کاتالوگ عمومی). کسر واقعی از wallet هنوز وصل نشده — همون gap اجرای completion واقعی که CP-038 هم داشت. جزئیات: docs/spect/DONE.md                                                                       |
...
| CP-044 | ✅ دسته‌بندی کاربردی مدل‌ها (الهام از openrouter.ai/models — مثلاً Roleplay/Programming/Marketing) — محور جدا از دسته‌بندی فنی CP-037                                                                                                                                                                                                                                                                                                                      | `functional_categories TEXT[]` روی کاتالوگ — آزاد (نه enum ثابت)، ادمین از فرم می‌تونه دسته‌ی جدید اضافه کنه بدون migration. جزئیات: docs/spect/DONE.md                                                                                                                    |
...
## بخش H — Relay وب‌هوک عمومی برای Runtimeهای لوکال/پشت NAT (جلسه ۱۰، `06_MEETINGS/10-public-webhook-relay-via-platform.md`)

> پس‌زمینه: `POST /v1/webhooks/:token` (RT-065) مستقیم روی Runtime گیت‌وی‌ه — کاربرانی
> که Runtime رو لوکال نصب می‌کنن نمی‌تونن ازش برای webhook (Telegram/Stripe/GitHub/…)
> استفاده کنن چون localhost عمومی نیست. تصمیم جلسه ۱۰: **relay نازک** — Platform منطق
> routing (agent/workflow/tool) رو دوباره نمی‌سازه، فقط یک لیبل (`name`) + یک
> `public_relay_token` جدا از توکن Runtime نگه می‌داره و payload رو از طریق یک
> WebSocket دائمی به همون Runtime instance، با همون token اصلی Runtime، forward
> می‌کنه. تسک‌های سمت Runtime (اتصال WebSocket + target مستقیم `tool`) در
> `docs/spect/TODO-openon4net-runtime.md` بخش L.
>
> **✅ تصمیم‌گیری شد (۲۰۲۶-۰۷-۲۰):** رفتار offline = `fail-fast (503)`. توسط Claude با
> تفویض صریح کاربر تصمیم‌گیری شد («خودت تصمیم بگیر... تا بعد درست چک کنم») —
> موقت-اما-فعال تا کاربر بعداً بازبینی کنه. جزئیات: `06_MEETINGS/10-*.md` Action Item ۶.
> **وابستگی activation:** این feature هیچ fallback self-host-only نداره (برخلاف
> RT-093's مسیر مستقیم Marketplace) — چون خودِ مشکل («localhost عمومی نیست») فقط
> با یک واسط عمومی (Platform) قابل حل‌ه. باید توی UI/مستندات صریح باشه که فقط برای
> سازمان‌های activated در دسترسه.

| #      | تسک                                                                                                                                                                                                                 | یادداشت                                                                                                                                                                                                                                                                                                                                   |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CP-045 | ✅ مدیریت webhook relay: جدول جدید (`org_id`, `name`, `runtime_webhook_token`, `public_relay_token`) + CRUD API + صفحه‌ی وب برای ساخت/حذف/دیدن لیست                                                                 | **تصحیح حین پیاده‌سازی (۲۰۲۶-۰۷-۲۰):** `runtime_webhook_token` نمی‌تونه فقط hash باشه — CP-047/RT-108 باید مقدارش رو دوباره forward کنن، پس AES-256-GCM رمزنگاری قابل‌بازگشت شد (مثل `ai_gateway_provider_configs`)؛ `public_relay_token` طبق سند اصلی HMAC-hash موند (bearer credential، فقط مقایسه می‌شه). جزئیات: `docs/spect/DONE.md` |
| CP-046 | WebSocket server در Platform که Runtime بهش وصل می‌شه — احراز هویت با همون activation key/security token موجود (الگوی CP-032)، نه یک مکانیزم auth جدا                                                               | یک اتصال به‌ازای هر Runtime instance (نه به‌ازای هر webhook relay) — چندتا relay روی همون یک کانال multiplex می‌شن                                                                                                                                                                                                                        |
| CP-047 | ✅ مسیر عمومی `POST /v1/webhook-relay/:publicToken` — بدون auth (خودِ توکن اعتبارسنجیه، مثل RT-065)، لوکاپ می‌کنه کدوم Runtime instance متصله، payload + `runtime_webhook_token` رو از طریق همون WebSocket می‌فرسته | رفتار offline: `fail-fast (503)` — تصمیم نهایی شد (بالا ببینید). جزئیات: `docs/spect/DONE.md`                                                                                                                                                                                                                                             |

---

## بخش I — اصلاح معماری ارتباطی (جلسه ۱۱، `06_MEETINGS/11-cross-plane-transport-architecture.md`)

> اصل کلی این جلسه: «هرجا ارتباط دوطرفه/push لازمه، WebSocket؛ وگرنه HTTP کافیه.» روی
> Platform↔Memory/Marketplace اعمال شد: عملیات ساده (submit/list/install/query) همون HTTP
> proxy موجود (CP-032/034) می‌مونه، فقط دو مسیر واقعاً «دوطرفه» (fine-tuning job status از
> Memory، sandbox test status از Marketplace) WebSocket جدید می‌گیرن.

| #      | تسک                                                                                                                                                                                                           | یادداشت                                                                             |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| CP-048 | WebSocket endpoint در Platform برای پوش بلادرنگ دو نوع رویداد به کاربر: وضعیت fine-tuning job (سمت Memory، `MEM-019`) و وضعیت sandbox test پلاگین (سمت Marketplace، `MKT-027`) — یک کانال مشترک، نه دو تا جدا | بلاک روی MEM-019 و MKT-027 (باید اول سمت Memory/Marketplace منبع رویداد داشته باشن) |

---

## بخش J — شارژ کیف‌پول + افیلیت + AI Gateway به‌عنوان Plugin (جلسه ۱۲، `06_MEETINGS/12-plugin-based-llm-providers-and-channels.md`)

| #      | تسک                                                                                                                                                                             | یادداشت                                                                                                                                                                                                  |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CP-049 | روش‌های شارژ کیف‌پول: درگاه بانکی خودکار + دستی (از قبل MVP-lite موجود) + کریپتو/استیبل‌کوین                                                                                    | گسترش عملیاتی CP-013 (Billing واقعی، از قبل به‌عنوان Later ثبت شده بود) — این تسک روش‌های مشخص پرداخت رو اضافه می‌کنه                                                                                    |
| CP-050 | سیستم افیلیت — معرف/کد ارجاع + پورسانت روی مصرف/خرید کاربر معرفی‌شده + **فرم ادمین برای تعیین درصد پورسانت** (تصمیم جلسه ۱۲: عدد ثابت hardcoded نه، ادمین از پنل تنظیمش می‌کنه) | جزئیات باز: سطح‌بندی (تک‌سطحی یا چندسطحی)، روش دقیق پرداخت پورسانت — نیاز به یک دور Q&A جدا قبل از پیاده‌سازی                                                                                            |
| CP-051 | ✅ بسته‌بندی «AI Gateway on4net» به‌عنوان یک پلاگین نصب‌شدنی رسمی                                                                                                               | ساخته و تست شد (۲۰۲۶-۰۷-۲۹) — ثبت در رجیستری CP-052 (`ai-gateway`) + صفحه‌ی وب `/plugins/ai-gateway` (انتخاب org→workspace→issue/list/revoke token، روی RT-101/CP-036 موجود). جزئیات: docs/spect/DONE.md |

---

## بخش K — Platform Plugin Hosting + Channel/Google plugins + بلاگ (جلسه ۱۵، `06_MEETINGS/15-media-channel-google-plugins-and-blog.md`)

> پس‌زمینه کامل در فایل جلسه بالا. CP-052 پایه‌ی همه‌ی تسک‌های زیر این بخشه —
> امروز Platform هیچ مکانیزم نصب/ثبت پلاگینی نداره (فقط route ثابت). تسک‌های سمت
> Runtime در `TODO-openon4net-runtime.md` بخش P.

| #      | تسک                                                                                                                                                                                     | یادداشت                                                                                                                                                                                                                                                                                                  |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CP-052 | ✅ **Platform Plugin Hosting System** — mechanism نصب/ثبت manifest-driven برای پلاگین‌های سمت Platform + ثبت پویا در منوی وب                                                            | v1: رجیستری first-party (نه بارگذاری پویای کد شخص‌ثالث — تصمیم آگاهانه، جزئیات در DONE.md) + toggle enable/disable ادمین + API منوی وب. هنوز هیچ پلاگین واقعی ثبت نشده (رجیستری خالیه) — پایه‌ی CP-054/056+                                                                                              |
| CP-053 | ✅ تعمیم CP-046 (WebSocket relay وب‌هوک) به یک «Plugin Event Relay» عمومی — همون اتصال دائمی Runtime↔Platform، چند نوع رویداد (webhook + پیام channel + …) روی یک کانال multiplex می‌شن | مسیر `/v1/webhook-relay/ws` به `/v1/plugin-relay/ws` تغییر نام داد + `PluginEventFrame` مشترک (`{type, data, sentAt}`) اضافه شد. جزئیات: docs/spect/DONE.md                                                                                                                                              |
| CP-054 | ✅ **پلاگین Google (جایگزین CP-041)** — یک پلاگین، consent انتخابی چند سرویس (Drive/Calendar/Gmail/Sheets)، چند حساب هم‌زمان (one-to-many)                                              | ساختار/schema/OAuth flow کامل ساخته و تست شد (با client fake — واقعی نیاز به credential دارد). جزئیات: docs/spect/DONE.md                                                                                                                                                                                |
| CP-055 | ✅ (بکند‌اند) بخش بلاگ چندزبانه در Platform — نویسندگی توسط ادمین، سئو (slug/meta per-language)، لینک به صفحه‌ی کاتالوگ provider/model (CP-037/CP-040)                                  | backend کامل ساخته و تست شد (schema + service + routes + AI translate). **وب UI ساخته نشد** (صفحه‌نویسی ادمین + صفحات عمومی بلاگ) — یک follow-up جدا. جزئیات: docs/spect/DONE.md                                                                                                                         |
| CP-056 | ✅ Telegram Channel Plugin (سمت Platform) — دریافت inbound از Telegram Bot API، forward از طریق CP-053                                                                                  | ساخته و تست شد (bot token، نه OAuth). جزئیات: docs/spect/DONE.md                                                                                                                                                                                                                                         |
| CP-057 | ✅ WhatsApp Channel Plugin (سمت Platform)                                                                                                                                               | ساخته و تست شد. WABA id + Phone Number id + توکن دستی (سیستم-یوزر)، وب‌هوک ورودی از طریق همون callback مشترک Meta (facebook-plugin.ts، دیسپچ بر اساس فیلد `object`). جزئیات: docs/spect/DONE.md                                                                                                          |
| CP-058 | ⚠️ Discord Channel Plugin (سمت Platform) — **نیاز به تصمیم معماری جدا قبل از build**                                                                                                    | وابسته به CP-052 + CP-053. نیمه‌ی Platform جفت RT-142. برخلاف Telegram/Meta/Google، پیام‌های معمولی Discord نیاز به یک اتصال WebSocket Gateway **دائمی و خروجی** (نه یک وب‌هوک ورودی) دارن — یک الگوی معماری کاملاً جدید که هیچ جلسه‌ای تصمیمش نگرفته. جزئیات در docs/spect/DONE.md (یادداشت ۲۰۲۶-۰۷-۲۳) |
| CP-059 | ⚠️ Google Chat Channel Plugin (سمت Platform) — **نیاز به تصمیم معماری جدا قبل از build**                                                                                                | وابسته به CP-052 + CP-053. نیمه‌ی Platform جفت RT-143. تأیید هویت وب‌هوک Google Chat مبتنی بر JWT audience-check‌ه (نه secret ساده مثل بقیه) — تصمیم امنیتی که نیاز به بررسی جدا داره                                                                                                                    |
| CP-060 | ⚠️ WeChat Channel Plugin (سمت Platform) — **نیاز به تصمیم معماری جدا قبل از build**                                                                                                     | وابسته به CP-052 + CP-053. نیمه‌ی Platform جفت RT-144. پیام‌های WeChat به‌صورت XML+امضای اختصاصی هستن (نه JSON مثل بقیه) — تصمیم معماری جدا لازمه                                                                                                                                                        |
| CP-061 | ⚠️ iMessage Channel Plugin (سمت Platform) — **نیاز به تحقیق feasibility قبل از build**                                                                                                  | همون محدودیت RT-145 (هیچ API عمومی اپل نیست)                                                                                                                                                                                                                                                             |

> **افزوده‌ی همون روز (جلسه ۱۵، بخش ۶):** Facebook/Instagram جفت کامل (مثل
> Channel)؛ TikTok/LinkedIn/YouTube چون پلتفرم انتشار محتوان نه گفتگو، فقط
> یک پلاگین Platform-side دارن (الگوی Google، بدون جفت Runtime).

| CP-062 | ✅ Facebook Messenger Channel Plugin (سمت Platform) | ساخته و تست شد. v1 ساده‌شده: Page Access Token دستی (مثل الگوی Telegram)، نه OAuth چندمرحله‌ای انتخاب Page. جزئیات: docs/spect/DONE.md |
| CP-063 | ✅ Instagram DM Channel Plugin (سمت Platform) | ساخته و تست شد. Page Access Token دستی (Page متصل به IG Business Account)، اشتراک وب‌هوک مشترک با Facebook (همون subscribed_apps سطح Page)، وب‌هوک ورودی از طریق همون callback مشترک Meta. جزئیات: docs/spect/DONE.md |
| CP-064 | ✅ TikTok Connector Plugin (فقط سمت Platform، بدون جفت Runtime) | ساخته و تست شد. OAuth app جدا (TikTok for Developers)، الگوی پلاگین Google. جزئیات: docs/spect/DONE.md |
| CP-065 | ✅ LinkedIn Connector Plugin (فقط سمت Platform، بدون جفت Runtime) | ساخته و تست شد. فاز اول فقط publish/read (⚠️ API رسمی messaging محدود/partner-only)، بدون refresh token (رفتار واقعی LinkedIn). جزئیات: docs/spect/DONE.md |
| CP-066 | ✅ YouTube Connector Plugin (فقط سمت Platform، بدون جفت Runtime) | ساخته و تست شد. همون OAuth app گوگل (CP-054) با redirect URI جدا، بدون DM (⚠️ اصلاً DM نداره). جزئیات: docs/spect/DONE.md |

> **معماری «دو دسته پلاگین» (تصمیم جلسه ۱۵، تصحیح‌شده همون روز):** هر پلاگین
> دقیقاً یکی از این دو `installTarget` رو داره — **Platform** (برای توسعه/
> زیرساخت) یا **Runtime** (تنها دسته‌ی قابل‌فروش، از طریق Marketplace). CP-051
> (بسته‌بندی AI Gateway) اولین نمونه‌ی این الگو بود. **تصحیح:** فرض اولیه («پلاگین
> Platform هیچ‌وقت روی Marketplace لیست نمی‌شه») اشتباه بود — همه‌ی پلاگین‌هایی
> که برای Runtime استفاده می‌شن (چه installTarget=platform چه runtime، چه رایگان
> چه قابل‌فروش) روی Marketplace لیست می‌شن؛ فقط پلاگین‌های کاملاً داخلی Platform
> (CP-052/CP-053/CP-055) مستثنی‌ان. این یعنی هم فیلد `installTarget` به schema
> manifest (RT-119) اضافه می‌شه، هم یک تسک enforcement جدید سمت Marketplace لازمه
> (MKT-032، `TODO-openon4net-marketplace.md` بخش J).
