...
## بخش H — تست بلادرنگ + مشارکت داوطلبانه (جلسه ۱۱، `06_MEETINGS/11-cross-plane-transport-architecture.md`)

> پس‌زمینه: MKT-025 (sandbox test-gate خودکار + AI agent) از قبل ساخته شده. جلسه ۱۱ دو
> چیز اضافه کرد: (۱) طبق اصل کلی «هرجا ارتباط دوطرفه لازمه، WebSocket»، وضعیت تست باید
> بلادرنگ push بشه نه فقط poll، (۲) یک مسیر موازی برای مشارکت **انسان‌های داوطلب** در تست،
> قبل از انتشار عمومی — جزئیات دقیقش هنوز باز است (جلسه‌ی بعدی نیاز داره).

| #       | تسک                                                                                                                                                                            |                                                                         نیاز به اجازه فایل ریشه؟                                                                         | وضعیت |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :---: |
| MKT-027 | وضعیت sandbox test (روی MKT-025 موجود) از طریق یک event source بلادرنگ قابل push باشه — سمت واقعیِ push به کاربر در CP-048 (Platform) ساخته می‌شه، این تسک فقط سمت Marketplace |                                                                                  ❌ خیر                                                                                  |  ⏳   |
| MKT-028 | مسیر مشارکت داوطلبانه‌ی انسانی در تست پلاگین قبل از انتشار عمومی                                                                                                               | ❌ خیر — **جزئیات در جلسه ۱۲ روشن شد** (پایین، بخش H۲): پلاگین‌های در انتظار تست به داوطلبان نمایش داده می‌شه، رایگان، جایزه برای گزارش باگ. پیاده‌سازی واقعی در MKT-029 |  ⏳   |

## بخش H۲ — تکمیل MKT-028 (جلسه ۱۲، `06_MEETINGS/12-plugin-based-llm-providers-and-channels.md`)

| #       | تسک                                                                                                                                                 | نیاز به اجازه فایل ریشه؟ | وضعیت |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------: | :---: |
| MKT-029 | پیاده‌سازی کامل مسیر تست داوطلبانه: ثبت‌نام کاربر به‌عنوان داوطلب تست + نمایش پلاگین‌های در انتظار تست بهش + نصب/نظردهی رایگان + گزارش باگ با جایزه |          ❌ خیر          |  ⏳   |

## بخش I — تقویت enforcement استاندارد manifest پلاگین (جلسه ۱۳، `06_MEETINGS/13-plugin-manifest-standard-enforcement.md`)

> پس‌زمینه کامل + تصمیم‌ها در `TODO-openon4net-runtime.md` بخش O. این تسک فقط سمت
> Marketplace است: مسیر submit همون schema مشترکی رو enforce می‌کنه که Runtime هم
> استفاده می‌کنه، تا یک پلاگین منتشرشده هیچ‌وقت جایی که Runtime قبولش نمی‌کنه رد نشه.

| #       | تسک                                                                                                                                     | نیاز به اجازه فایل ریشه؟ | یادداشت                                                                                                 | وضعیت |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------- | :----------------------: | ------------------------------------------------------------------------------------------------------- | :---: |
| MKT-030 | اعمال `PluginManifestSchema` (از `@o2n/plugin-sdk`) روی مسیر submit (`routes/publisher-plugins.ts`) — رد با خطای تمیز به‌جای پذیرش blob |    ✅ بله (غیرمستقیم)    | وابسته به RT-118 (ثبت این ریپو در root workspace) + RT-119 (خودِ schema)، هر دو در `openon4net-runtime` |  ⏳   |

## بخش J — گسترش دسته‌بندی پلاگین + پشتیبانی نصب `installTarget=platform` (جلسه ۱۵، `06_MEETINGS/15-media-channel-google-plugins-and-blog.md`)

> پس‌زمینه کامل در `TODO-openon4net-runtime.md` بخش P. **تصحیح‌شده همون روز
> (بخش ۳ موضوع ۷ سند جلسه):** فرض اولیه («پلاگین‌های Platform هیچ‌وقت اینجا لیست
> نمی‌شن») اشتباه بود. تصمیم درست: **همه‌ی پلاگین‌هایی که برای Runtime استفاده
> می‌شن — چه installTarget=runtime چه installTarget=platform، چه رایگان چه
> قابل‌فروش — روی Marketplace لیست می‌شن.** فقط پلاگین‌های کاملاً داخلی Platform
> (CP-052 خودِ hosting، CP-053 خودِ relay، CP-055 بلاگ) مستثنی‌ان.

| #       | تسک                                                                                                                                                                                                                                                                                   | نیاز به اجازه فایل ریشه؟ | یادداشت                                                    | وضعیت |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------: | ---------------------------------------------------------- | :---: |
| MKT-031 | هم‌گام‌سازی enum دسته‌بندی پلاگین با RT-132 — افزودن `llm-provider`, `media-generation`, `connector` به CHECK constraint (migration جدید، طبق الگوی migration 0006)                                                                                                                   |          ❌ خیر          | وابسته به RT-132 (Runtime باید هم‌زمان/هم‌شکل به‌روز بشه)  |  ⏳   |
| MKT-032 | پشتیبانی نصب `installTarget=platform` — مسیر «نصب» Marketplace باید بر اساس `installTarget` شاخه بره (دانلود فایل Runtime در مقابل راه‌انداختن فرآیند فعال‌سازی/OAuth سمت Platform)؛ برای Channelهای جفتی (Telegram/…) یک ردیف Marketplace واحد کل قابلیت رو نشون بده، نه دو ردیف جدا |          ❌ خیر          | وابسته به RT-119 (فیلد `installTarget` در manifest schema) |  ⏳   |
