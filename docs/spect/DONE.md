...
- **RBAC — Policy Layer (ABAC، §6 سند `10-rbac-and-policy.md`):** جدول `roles`/`role_permissions`/`user_role_bindings` («minimum» §4/§8) و UI مدیریت اون ساخته و تست شده (بالا). جدول جداگانه‌ی `policies` با شرایط ABAC (cost/layer/tag/time/environment) هنوز ساخته نشده. همچنین: ساخت نقش سفارشی (فراتر از ۴ نقش سیستمی seed‌شده)، حذف نقش، و UI تخصیص نقش به کاربر (`user_role_bindings`) هنوز نیست — فقط ویرایش permissions نقش‌های موجود.
