...
- **`openon4net-control-plane` (Plane 2) از 2026-07-09 شروع شد.** این سند فقط Runtime را پوشش می‌دهد؛ بک‌لاگ/اسپرینت‌های Control Plane (شامل T-011 منتقل‌شده) در `docs/sprint-plan/04_control-plane-backlog.md` است.
...
> **⚠️ T-011 منسوخ برای Runtime.** طبق تصمیم معماری بعدی (بخش «وضعیت واقعی» بالای این فایل)، routing/failover بین چند مدل مسئولیت `openon4net-control-plane` است، نه `openon4net-runtime`. این task منتقل شده به `docs/sprint-plan/04_control-plane-backlog.md` (Epic E-063، تسک‌های T-CP-015..T-CP-019، اسپرینت CP-SP-03).
