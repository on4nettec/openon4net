...
| CP-029 | قفل activation key سازمانی به IP استاتیک/دامنه — چک در هر check-in، عدم تطابق = رد                                                        | به CP-026 وابسته است (نوع سازمانی باید مشخص باشه)                                                                      |
| CP-030 | Revoke/expire واقعی — route ادمین برای لغو دستی + اجرای واقعی `expires_at`                                                                | مستقل — الان `status`/`expires_at` توی schema هستن ولی هیچ کدی تغییرشون نمی‌ده                                         |
