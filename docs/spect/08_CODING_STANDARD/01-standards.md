...
async function getAgentMemory(
  ...
  // implementation
  const results = await Promise.all(layers.map((layer) => queryLayer(agentId, layer)));
...
export class BudgetExceededError extends O2NError {
  constructor(agentId: string, budget: number) {
    super('BUDGET_EXCEEDED', `Agent ${agentId} exceeded monthly budget of $${budget}`, 402);
  }
}
...
describe('MemoryEngine', () => {
  ...
  describe('query()', () => {
    ...
    it('should throw for invalid layer', async () => {
      await expect(engine.query('agent-1', [99])).rejects.toThrow(ValidationError);
    });
  });
});
```
